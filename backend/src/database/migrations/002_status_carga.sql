...
DECLARE
  v_carga_id INTEGER;
  v_old_carga_id INTEGER;
BEGIN
  IF TG_TABLE_NAME = 'entregas' THEN
    v_carga_id := COALESCE(NEW.carga_id, (SELECT id FROM cargas WHERE carga = NEW.fc AND transportadora_id = NEW.transportadora_id LIMIT 1));
    IF OLD.fc IS DISTINCT FROM NEW.fc THEN
      v_old_carga_id := (SELECT id FROM cargas WHERE carga = OLD.fc AND transportadora_id = OLD.transportadora_id LIMIT 1);
    END IF;
  ELSIF TG_TABLE_NAME = 'cargas' THEN
    v_carga_id := NEW.id;
  END IF;

  IF v_carga_id IS NOT NULL THEN PERFORM atualizar_status_carga(v_carga_id); END IF;
  IF v_old_carga_id IS NOT NULL AND v_old_carga_id IS DISTINCT FROM v_carga_id THEN
    PERFORM atualizar_status_carga(v_old_carga_id);
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
AFTER INSERT OR UPDATE OF confirma_entrega, carga_id, fc ON entregas
